...
class GroundedSAM:
    ...
    def get_video_mask(self, video, text, initial_threshold, redetect_interval, visualize=False):
        ...
        for start_frame_idx in range(0, len(frame_names), redetect_interval):

            mask_dict = MaskDictionaryModel(
                promote_type = PROMPT_TYPE_FOR_VIDEO, 
                mask_name = f"mask_{start_frame_idx}.npy")
            masks, results = self.get_image_mask(
                image=video[start_frame_idx],
                text=text,
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                for object_id, object_info in mask_dict.labels.items():
                    frame_idx, out_obj_ids, out_mask_logits = self.sam2_video_predictor.add_new_mask(
                            inference_state,
                            start_frame_idx,
                            object_id,
                            object_info.mask,
                ...
                for out_frame_idx, out_obj_ids, out_mask_logits in self.sam2_video_predictor.propagate_in_video(
                    inference_state, 
                    max_frame_num_to_track=redetect_interval, 
